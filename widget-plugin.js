...
function CreateWhatsAppButtonAndWidget(options) {
			...
			: ""
				...
				<div class="df-content-bottombar">
					...
					<div class="df-bottombar-branding">
						...
						/>
						by 
						<a href="https://jagoankode.com" target="_blank"> <i>CV JAGOAN KODE GROUP INDONESIA</i> </a>
					</div>
				</div>
			</div>
	...
	window.goToLink = () => {
		...
		if (config.env != "dev" && window.location.hostname != "create.aisensy.com") {
			let redirectUrl = `https://wa.aisensy.com/${config.linkid}?url=${window.location.href}&title=${document.title}`;
			// let redirectUrl = `http://localhost:5500/${config.linkid}?url=${window.location.href}&title=${document.title}`;
			if (getGclid()) {
				redirectUrl += `&gclid=${getGclid()}`;
			}
